...
def conecta_db():
  con = psycopg2.connect (host = cfg.get('banco de dados', 'host'), 
                         database = cfg.get('banco de dados', 'database'),
                         user = cfg.get('banco de dados', 'user'),
                         password = cfg.get('banco de dados', 'password'))
  return con
...
def consulta(data):
    consulta = montaQueryConsultaMovimentosDia(data)
    resultado, cabecalho = consultar_db(consulta)
    return resultado, cabecalho

def consultaPorArquivo(arquivo_consulta):
    with open(arquivo_consulta,'r') as arq:
        consulta = arq.read()
        resultado, cabecalho = consultar_db(consulta)
    return resultado, cabecalho


def monta_query_consulta_movimentos_dia_teste(data):
   consulta = (f'select * from abc where a between \'{data} 00:00:00.000\' and \'{data} 23:59:59.999\'')
   return consulta

def montaQueryConsultaMovimentosDia(data):
   consulta = (
      f"""
      select 
      ...
      from
        ...
        where
          ...
          -------------------------> ################## DATAS PARA SELEÇÃO ##########################################################
          WHERE
            tsk.tsk_lastexecutiondatehour between  '{data} 00:00:00.000' and '{data} 23:59:59.999' and
